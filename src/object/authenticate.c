 ...
 */
static int
au_grant_internal (MOP user, MOP class_mop, DB_AUTH type, bool grant_option)
{
  int error = NO_ERROR;
  MOP auth;
  DB_SET *grants;
  ...
  if (catcls_Enable == true)
#endif /* SA_MODE */
    {
      ...
      if (ins_bits)
	{
	  error = au_insert_new_auth (Au_user, user, class_mop, ins_bits, (grant_option) ? ins_bits : false);
	  if (error != NO_ERROR)
	    {
	      set_free (grants);
	      return error;
      ...
      if (upd_bits)
	{
	  error = au_update_new_auth (Au_user, user, class_mop, upd_bits, (grant_option) ? upd_bits : false);
	  if (error != NO_ERROR)
	    {
	      set_free (grants);
	      return error;
  ...
  /* Make sure any cached parse trees are rebuild.  This proabably isn't necessary for GRANT, only REVOKE. */
...
/*
 * au_grant - This is the primary interface function for granting authorization on classes.
 *   return: error code
 *   user(in): user receiving the grant
 *   class_mop(in): class being authorized
 *   type(in): type of authorization
...
{
  int save = 0, error;
  int is_partition = DB_NOT_PARTITIONED_CLASS, i;
  bool savepoint_grant = false;
  ...
  if (is_partition == DB_PARTITIONED_CLASS)
	...
	{
	  error = au_grant_internal (user, sub_partitions[i], type, grant_option);
	  if (error != NO_ERROR)
	    {
	      goto end;
	    }
  ...
  error = au_grant_internal (user, class_mop, type, grant_option);
  if (error != NO_ERROR)
    {
      goto end;
    }
  ...
  if (savepoint_grant && error != NO_ERROR && !ER_IS_ABORTED_DUE_TO_DEADLOCK (error))
    {
      (void) tran_abort_upto_system_savepoint (UNIQUE_SAVEPOINT_GRANT_USER);
    }
